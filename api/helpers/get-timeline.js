...
module.exports = {
  ...
  fn: async function(inputs, exits) {
    ...
    if (isCovidLiveUpdatePage({ url })) {
      ...
      return exits.success(
        JSON.stringify({
          articles: articles,
          terms: ['coronavirus'],
          timeline: { id: 'covid-live-update', method: 'covid aggregated' },
          original_article: {
            title: 'Live Update on COVID-19',
            url: url,
            body: '',
            publishedAt: Date.now(),
            terms: [{ text: 'Coronavirus', type: 'other' }]
          }
    ...
    if (originalArticle.timeline.length > 0) {
      ...
      let timelineExists = await Timelines.find({ where: { creatorId: originalArticle.id }, sort: 'createdAt DESC' });

      let mostRecentlyUpdated;
      if (timelineExists[0].updatedAt) {
        mostRecentlyUpdated = new Date(timelineExists[0].updatedAt);
      } else {
        mostRecentlyUpdated = new Date(timelineExists[0].createdAt);
      }
      console.log(`now: ${new Date().toISOString()}  ---converted back: ${new Date(new Date().toISOString())}`);
      let oneDay = 1000 * 60 * 60 * 24; //one day max?
      let hasntBeenUpdatedFor = new Date() - mostRecentlyUpdated;
      //removed substraction of one day from mostRecentlyUpdated
      console.log(`hasntBeenUpdatedFor: ${hasntBeenUpdatedFor / (1000 * 60 * 60)} hours`);

      if (hasntBeenUpdatedFor <= oneDay) {
        ...
        return exits.success(
          JSON.stringify({
            articles: articles,
            terms: removeUnnecessaryTerms(_cullTerms(originalArticle.terms)),
            timeline: { id: timelineId, method: timelineExists[0].method },
            original_article: formatForTimeline(originalArticle)
          })
        );
      } else {
        console.log('recreating timeline');
        return exits.success(null);
      }
    }
    console.log('get-timeline - NO TIMELINE FOUND');
    return exits.success(null);
  }
};
...
async function getPublisherNames(articles) {
  ...
  for (let p = 0; p < pubsUniques.length; p++) {
    ...
    if (pubUid) {
      ...
      console.log(`no pubUid: ${p}`, pubsUniques);
    }
  }
  for (let i = 0; i < articles.length; i++) {
    let item = articles[i];
...
async function getArticlesFromTimelineId(timelineId, originalArticleUrl) {
  ...
  console.log(`article ids found for timeline: ${articleIds.length}`);
...
async function getAllCustomCovidTimelines(hostArticle) {
  ...
  for (var i in customCovidTimelineIds) {
    ...
    timeline = timeline.concat(articles);
  }
